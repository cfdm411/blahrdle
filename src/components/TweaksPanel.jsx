...
const PANEL_STYLE = `
  .twk-panel{position:fixed;right:16px;bottom:16px;z-index:2147483646;width:280px;
    ...
    font:11.5px/1.4 'Outfit',ui-sans-serif,system-ui,sans-serif;overflow:hidden}
...
export function useTweaks(defaults) {
  const [values, setValues] = useState(() => {
    try {
      const stored = localStorage.getItem('lordle_tweaks')
  ...
  const setTweak = useCallback((keyOrEdits, val) => {
    ...
    setValues(prev => {
      ...
      try { localStorage.setItem('lordle_tweaks', JSON.stringify(next)) } catch {}
      return next
    })
  }, [])
